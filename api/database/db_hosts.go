...
func ImportInventoryToDB(ctx context.Context, hs []common.Host) error {
	return UpsertHosts(ctx, hs)
}

// Upsert hosts parsed from inventory into DB.
func UpsertHosts(ctx context.Context, items []common.Host) error {
	for _, h := range items {
		if h.Vars == nil {
