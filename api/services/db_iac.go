...
func pruneIacStacksNotIn(ctx context.Context, repoID int64, keepIDs []int64) (int64, error) {
	...
	if len(keepIDs) == 0 {
		cmd, err := common.DB.Exec(ctx, `DELETE FROM iac_stacks WHERE repo_id=$1`, repoID)
		if err != nil {
			return 0, err
		}
		return cmd.RowsAffected(), nil
	}
	cmd, err := common.DB.Exec(ctx, `DELETE FROM iac_stacks WHERE repo_id=$1 AND id <> ALL($2)`, repoID, keepIDs)
	if err != nil {
		return 0, err
	}
	return cmd.RowsAffected(), nil
...
func listIacStacksForHost(ctx context.Context, hostName string) ([]IacStackOut, error) {
	common.DebugLog("listIacStacksForHost called for host: %s", hostName)
	h, err := database.GetHostByName(ctx, hostName)
	if err != nil {
		common.DebugLog("GetHostByName failed for %s: %v", hostName, err)
		return nil, err
	}
	...
	common.DebugLog("Host %s belongs to groups: %v", hostName, groups)
	...
	for _, s := range stacks {
		...
		if err != nil {
			common.DebugLog("Failed to check stack content for %s: %v", s.Name, err)
			continue
		}
		if !hasContent {
			common.DebugLog("Skipping empty stack %s (no files)", s.Name)
			continue
		}
		filteredStacks = append(filteredStacks, s)
	}
	stacks = filteredStacks
	common.DebugLog("After filtering empty stacks: %d stacks remain for host %s", len(stacks), hostName)

	// load services per stack
	for i := range stacks {
		rs, err := common.DB.Query(ctx, `
...
func DeleteIacFileRow(ctx context.Context, stackID int64, relPath string) error {
	_, err := common.DB.Exec(ctx, `DELETE FROM iac_stack_files WHERE stack_id=$1 AND rel_path=$2`, stackID, relPath)
	return err
}
...
func StackHasContent(ctx context.Context, stackID int64) (bool, error) {
	var n int64
	if err := common.DB.QueryRow(ctx, `SELECT COUNT(1) FROM iac_stack_files WHERE stack_id=$1`, stackID).Scan(&n); err != nil {
		return false, err
	}
	if n > 0 {
		return true, nil
	...
	var compose string
	_ = common.DB.QueryRow(ctx, `SELECT COALESCE(compose_file,'') FROM iac_stacks WHERE id=$1`, stackID).Scan(&compose)
...
func ListEnhancedIacStacksForHost(ctx context.Context, hostName string) ([]EnhancedIacStackOut, error) {
	common.DebugLog("listEnhancedIacStacksForHost called for host: %s", hostName)
	base, err := listIacStacksForHost(ctx, hostName)
	if err != nil {
		common.DebugLog("listIacStacksForHost failed for %s: %v", hostName, err)
		return nil, err
	}
	common.DebugLog("Found %d base stacks for host %s", len(base), hostName)
	...
	if err != nil {
		common.DebugLog("Failed to create docker client for host %s: %v", hostName, err)
		return nil, err
	}
	defer cli.Close()
	common.DebugLog("Docker client created successfully for host %s", hostName)
	...
	for _, s := range base {
		...
		// Create wrapper function to adapt StageStackForCompose to StageStackFunc interface
		stageFunc := func(ctx context.Context, stackID int64) (string, interface{}, func(), error) {
		...
		if derr == nil {
			...
			if rs, rerr := renderComposeServices(ctx, stageDir, s.Name, stagedComposes); rerr == nil {
				...
				common.DebugLog("Stack %s rendered %d services", s.Name, len(rs))
				for _, r := range rs {
					common.DebugLog("Service %s -> image %s, container %s", r.ServiceName, r.Image, r.ContainerName)
				}
				common.DebugLog("Stack %s RenderedServices field populated with %d items", s.Name, len(e.RenderedServices))
			} else {
				common.DebugLog("Stack %s render services failed: %v", s.Name, rerr)
			}
		} else {
			common.DebugLog("Stack %s staging failed: %v", s.Name, derr)
		}
